...
for(p.mr in p.mrs){
  for(ltfur in ltfurs){
    ...
    png(paste("bias.x.emr~ss(",p.mr,')/',"mr~ss(",ltfur,',',min(pers.m.in.ltfus),'~',max(pers.m.in.ltfus),').png',sep=''),width=957,height=581)
    par(mfrow=c(2,2))
    ...
    png(paste("bias.x.emr~ss(",p.mr,')/',"bias~ss(",ltfur,',',min(pers.m.in.ltfus),'~',max(pers.m.in.ltfus),').png',sep=''),width=957,height=581)
    par(mfrow=c(2,2))
